...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'video-variator',https:req.secure||!isProduction,time:new Date().toISOString()}));
app.get('/api/version',(req,res)=>res.json({webVersion:process.env.WEB_VERSION||'3.0.0',androidVersion:process.env.ANDROID_VERSION||'3.0.0',androidVersionCode:Number(process.env.ANDROID_VERSION_CODE||3),latestApkUrl:process.env.LATEST_APK_URL||''}));
app.get('/api/config',(req,res)=>res.json({
  ...
  introOfferText:process.env.INTRO_OFFER_TEXT||'Intro discount available on your first subscription',privacy:{httpsRequired:isProduction,localVideoProcessing:true},plans:{basic:{price:9,limit:750,unit:'credits'},pro:{price:24,limit:2250,unit:'credits'},business:{price:99,limit:15000,unit:'credits'}}
}));

app.post('/api/auth/google',async(req,res)=>{try{if(!process.env.GOOGLE_CLIENT_ID)return res.status(503).json({error:'GOOGLE_AUTH_NOT_CONFIGURED'});const credential=req.body?.credential;if(!credential)return res.status(400).json({error:'MISSING_GOOGLE_CREDENTIAL'});const {payload}=await jwtVerify(credential,googleJwks,{audience:process.env.GOOGLE_CLIENT_ID,issuer:['https://accounts.google.com','accounts.google.com']});const user=await upsertUser({provider:'google',providerSub:payload.sub,email:payload.email,name:payload.name});res.json({token:await signAppToken(user),user:publicUser(user)});}catch(e){console.error('[google auth]',e);res.status(401).json({error:'GOOGLE_AUTH_FAILED'});}});
