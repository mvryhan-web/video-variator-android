(() => {
  ...
  async function fetchWithRetry(url,attempts=3){
    ...
    for(let i=0;i<attempts;i++){
      ...
      try{
        ...
        return r;
      }catch(e){clearTimeout(timer);last=e;if(i<attempts-1)await new Promise(r=>setTimeout(r,700*(i+1)));}
    }
    throw new Error(`Video engine download failed: ${last?.message||'network error'}`);
  }
  async function toBlobURL(url,type){const r=await fetchWithRetry(url);return URL.createObjectURL(new Blob([await r.arrayBuffer()],{type}));}
  async function ensureEngine(){
    ...
    }catch(e){hooks.onEngine('error');try{ffmpeg.terminate();}catch(_){ }throw new Error(`Could not start the local video engine. ${e?.message||e}`);}
  }
  ...
  async function processVariant(ffmpeg,input,file,duration,fileIndex,variantIndex,total,options,audioHint){
    ...
    if(audio){
      try{const a=args(input,internal,duration,r,w,h,true);log('> ffmpeg '+a.join(' '));const code=await ffmpeg.exec(a);if(code!==0)throw new Error('Video processing failed.');}
      catch(err){log('Audio track retry disabled for this source.');audio=false;try{await ffmpeg.deleteFile(internal)}catch(_){ }const a=args(input,internal,duration,r,w,h,false),code=await ffmpeg.exec(a);if(code!==0)throw err;}
    }else{const a=args(input,internal,duration,r,w,h,false),code=await ffmpeg.exec(a);if(code!==0)throw new Error('Video processing failed.');}
