(() => {
  ...
  async function toBlobURL(url,type){
    ...
    if(!r.ok) throw new Error(`Video engine download failed (${r.status})`);
  ...
  function durationOf(file){
    return new Promise((resolve,reject)=>{
      ...
      v.preload='metadata';
      v.onloadedmetadata=()=>{ const d=v.duration; URL.revokeObjectURL(url); Number.isFinite(d)&&d>0?resolve(d):reject(new Error('Could not read the video duration.')); };
  ...
  function filters(r,w,h,audio){
    ...
    if(audio){
      ...
      if(r.useCut){const a=fmt(r.cutAt,3),b=fmt(r.cutAt+r.cutLen,3);af.push(`aselect='not(between(t\\,${a}\\,${b}))'`,'asetpts=N/SR/TB');}
  ...
  function chunkDataURL(blob){
    return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(',')[1]||'');reader.onerror=()=>reject(reader.error||new Error('Could not read the output file.'));reader.readAsDataURL(blob);});
  }
  ...
  async function processOne(file,fileIndex,variantIndex,total,options){
    const ffmpeg=await ensureEngine();
    const duration=await durationOf(file),[w,h]=options.resolution.split('x').map(Number),r=recipe(duration,options.mode);
    const token=`${Date.now()}_${Math.random().toString(36).slice(2,7)}`,input=`in_${token}.${extOf(file.name)}`,internal=`out_${token}.mp4`;
    const name=`${safeBase(file.name)}_variant_${variantIndex+1}.mp4`,job=fileIndex*options.variants+variantIndex;
    hooks.onStage('process',{file:file.name,variant:variantIndex+1,job:job+1,total}); state.progress=0;
    await ffmpeg.writeFile(input,new Uint8Array(await file.arrayBuffer()));
    let audio=true;
    try{
      const a=args(input,internal,duration,r,w,h,true); log('> ffmpeg '+a.join(' ')); const code=await ffmpeg.exec(a); if(code!==0)throw new Error('Video processing failed.');
    }catch(err){
      log('Audio track retry disabled for this source.'); audio=false; try{await ffmpeg.deleteFile(internal)}catch(_){ }
      const a=args(input,internal,duration,r,w,h,false); const code=await ffmpeg.exec(a); if(code!==0)throw err;
    }
    ...
    let saved={saved:false,path:null}; try{saved=await saveAndroid(name,blob);}catch(e){log(e.message);}
    try{await ffmpeg.deleteFile(input)}catch(_){ } try{await ffmpeg.deleteFile(internal)}catch(_){ }
    return {id:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,sourceName:file.name,name,blob,saved:saved.saved,path:saved.path,resolution:`${w}×${h}`,createdAt:new Date().toISOString(),variant:variantIndex+1,audio};
  }
  ...
  async function process(options={}){
    ...
    const variants=Math.max(1,Math.min(5,Number(options.variants)||1));
    const mode=['gentle','balanced','dynamic'].includes(options.mode)?options.mode:'balanced';
    const resolution=/^(540x960|720x1280|1080x1920)$/.test(options.resolution||'')?options.resolution:'720x1280';
    ...
    try{
      hooks.onStage('load',{done,total}); await ensureEngine();
      for(let i=0;i<state.files.length;i++) for(let v=0;v<variants;v++){
        if(state.cancelled) throw new Error('Processing was canceled.');
        const result=await processOne(state.files[i],i,v,total,{variants,mode,resolution});
        results.push(result); done++; hooks.onProgress(done/total,{done,total});
      }
      hooks.onStage('done',{done,total}); return {results,sourceCount:state.files.length,outputCount:results.length};
    } finally { clearInterval(ticker); state.running=false; }
  }

  function cancel(){
    state.cancelled=true;
    if(state.ffmpeg&&state.running){try{state.ffmpeg.terminate()}catch(_){ }state.ffmpeg=null;state.loaded=false;hooks.onEngine('stopped');}
  }

  window.VideoVariatorCore={configure,setFiles,getFiles,process,cancel,state};
})();
