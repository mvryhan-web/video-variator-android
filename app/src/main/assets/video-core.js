(() => {
  ...
  async function ensureEngine(){
    ...
    await ffmpeg.load({classWorkerURL,coreURL,wasmURL});state.ffmpeg=ffmpeg;state.loaded=true;hooks.onEngine('ready');return ffmpeg;
  }
  ...
  async function estimateCredits(variants=1){
    const count=Math.max(1,Math.min(5,Number(variants)||1));
  ...
  async function processOne(file,duration,fileIndex,variantIndex,total,options){
    const ffmpeg=await ensureEngine(),[w,h]=options.resolution.split('x').map(Number),r=recipe(duration,options.mode),token=`${Date.now()}_${Math.random().toString(36).slice(2,7)}`,input=`in_${token}.${extOf(file.name)}`,internal=`out_${token}.mp4`,name=`${safeBase(file.name)}_variant_${variantIndex+1}.mp4`,job=fileIndex*options.variants+variantIndex;
    hooks.onStage('process',{file:file.name,variant:variantIndex+1,job:job+1,total});state.progress=0;await ffmpeg.writeFile(input,new Uint8Array(await file.arrayBuffer()));let audio=true;
    ...
    catch(err){log('Audio track retry disabled for this source.');audio=false;try{await ffmpeg.deleteFile(internal)}catch(_){ }const a=args(input,internal,duration,r,w,h,false);const code=await ffmpeg.exec(a);if(code!==0)throw err;}
    const data=await ffmpeg.readFile(internal),blob=new Blob([data.buffer],{type:'video/mp4'});hooks.onStage('save',{file:file.name,variant:variantIndex+1,job:job+1,total});let saved={saved:false,path:null};try{saved=await saveAndroid(name,blob);}catch(e){log(e.message);}try{await ffmpeg.deleteFile(input)}catch(_){ }try{await ffmpeg.deleteFile(internal)}catch(_){ }
  ...
  async function process(options={}){
    ...
    try{
      hooks.onStage('load',{done,total});const estimate=await estimateCredits(variants);await ensureEngine();
      for(let i=0;i<state.files.length;i++)for(let v=0;v<variants;v++){if(state.cancelled)throw new Error('Processing was canceled.');const result=await processOne(state.files[i],estimate.durations[i],i,v,total,{variants,mode,resolution});results.push(result);done++;hooks.onProgress(done/total,{done,total});}
  ...
  window.VideoVariatorCore={configure,setFiles,getFiles,durationOf,estimateCredits,process,cancel,state};
})();
