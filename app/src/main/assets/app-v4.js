(() => {
  ...
  const state={
    ...
    trialUsed:Math.max(0,Math.min(100,trialUsed)),estimate:null,history:readJson('vv_history',[]).slice(0,100),
  ...
  function usage(){if(state.user?.usage)return state.user.usage;return{plan:'trial',limit:100,used:state.trialUsed,remaining:Math.max(0,100-state.trialUsed),active:false,unit:'videos'};}
  function planName(id){return id==='basic'?'Basic':id==='pro'?'Pro':id==='business'?'Business':id==='lifetime'?'Lifetime':'Free trial';}
  function activePlan(){const u=usage();if(u.active&&u.allFeatures)return'lifetime';return u.active&&['basic','pro','business','lifetime'].includes(u.plan)?u.plan:'trial';}
  ...
  function renderHistory(){
    ...
    state.history.forEach(item=>{
      ...
      if(blob)historyActionButtons(item,blob,right);
        ...
        const badge=document.createElement('span');badge.className='historyBadge';badge.textContent=item.saved?t('historyStatusSaved'):t('historyStatusReady');right.appendChild(badge);if(window.VUResultDelete)right.append(window.VUResultDelete.button(item));
  ...
  window.addEventListener('vu-result-deleted',e=>{state.history=state.history.filter(x=>x.id!==e.detail.id);sessionDownloads.delete(e.detail.id);saveHistory();renderHistory();});
  ...
  async function cancelSubscription(){if(!state.user||!usage().active)return;if(usage().plan==='lifetime'||usage().allFeatures){toast('Lifetime access is permanent and has no recurring subscription to cancel.');return;}if(!confirm('Cancel this subscription at the end of the current billing period?'))return;try{await api('/api/billing/cancel',{method:'POST',body:'{}'});toast('Cancellation scheduled for the end of the current billing period.');await refreshAccount();}catch(e){reportError(e,'billing-cancel');toast(e.message);}}
  ...
  applyLocale();renderAll();if(location.hash==='#history')showView('history');loadConfig().then(refreshAccount).then(refreshAnalytics).then(async()=>{const q=new URLSearchParams(location.search);if(q.get("checkout")!=="success"||q.get("plan")!=="lifetime"||!state.user)return;for(let i=0;i<15;i++){if(usage().plan==="lifetime"&&usage().active){toast((navigator.language||"").startsWith("ru")?"Пожизненный доступ активирован.":"Lifetime access is active.");return;}await new Promise(r=>setTimeout(r,2000));await refreshAccount();}toast((navigator.language||"").startsWith("ru")?"Ожидаем подтверждение оплаты Stripe. Доступ появится после подтверждения.":"Waiting for Stripe payment confirmation. Access activates after confirmation.");});setupUpdates();
})();