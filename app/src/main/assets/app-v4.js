(() => {
  ...
  function initProviderButtons(){const c=state.config||{};if(c.googleClientId&&window.google?.accounts?.id){try{$('googleButton').innerHTML='';google.accounts.id.initialize({client_id:c.googleClientId,callback:async resp=>{try{const d=await api('/api/auth/google',{method:'POST',body:JSON.stringify({credential:resp.credential})});await authSuccess(d.token);}catch(e){reportError(e,'google-auth');toast(e.message);}}});google.accounts.id.renderButton($('googleButton'),{theme:'outline',size:'large',width:Math.max(200,Math.min(350,$('googleButton').clientWidth||250)),text:'continue_with'});}catch(e){console.warn(e);}}}
  document.querySelector('script[src*="accounts.google.com/gsi/client"]')?.addEventListener('load',initProviderButtons);
  ...
  $('signInBtn').addEventListener('click',()=>{if(state.user){state.token='';state.user=null;localStorage.removeItem('vv_token');window.google?.accounts?.id?.disableAutoSelect();renderAll();}else openAuth();});
