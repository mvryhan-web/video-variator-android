...
test('local avatar has measurable portrait motion with steady speech and stops during silence',async({},info)=>{
 ...
 const filter=buildAvatarFilter({w:160,h:284,envelope,mouth:{x:50,y:62,width:28}}),out=info.outputPath('moving-avatar.mp4');
 execFileSync('ffmpeg',['-y','-f','lavfi','-i','color=black:size=160x90:rate=24:duration=2','-f','lavfi','-i','testsrc2=size=160x120:rate=1:duration=1','-filter_complex',filter,'-map','[v]','-t','2','-c:v','libx264','-crf','0','-pix_fmt','yuv420p',out],{stdio:'ignore'});
