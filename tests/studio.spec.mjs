...
test('dashboard metrics are compact and pricing fits without changing prices',async({page})=>{
 ...
 await page.locator('.navBtn[data-view="plans"]').evaluate(e=>e.click());await expect(page.locator('#plansView')).toHaveClass(/active/);for(const price of ['$9','$24','$99'])await expect(page.locator('#plansView').getByText(price,{exact:true})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
});
test('real local models produce a transparent PNG, captions and a playable background video',async({page},info)=>{
 ...
 await page.locator('#aiFile').setInputFiles({name:'portrait.png',mimeType:'image/png',buffer:Buffer.from(png)});await page.locator('#aiRun').click();await expect(page.locator('#aiStatus')).toContainText('Done.',{timeout:240000});
 ...
 await page.locator('#aiTool').selectOption('video');await page.locator('#aiFile').setInputFiles({name:'video.mp4',mimeType:'video/mp4',buffer:readFileSync(mp4)});await page.locator('#aiRun').click();await expect(page.locator('#aiStatus')).toContainText('Done.',{timeout:120000});await expect.poll(()=>page.locator('#aiResult video').evaluate(v=>v.duration)).toBeGreaterThan(0);expect(posts).toBe(0);
});
